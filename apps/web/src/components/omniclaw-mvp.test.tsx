...
import { createOmniClawClient } from "@omniclaw/sdk";
...
describe("OmniClaw web MVP", () => {
  test("runs discovery, create, accept, submit, resolve, detail, and graph through the SDK", async () => {
    ...
    expect(ui.getByText(/Graph appears/)).toBeTruthy();
  });

  test("renders typed API error envelopes with code, message, path, and details", async () => {
    const { client } = await fixture("ui_error");
    const ui = render(<OmniClawMvp client={client} />);

    fireEvent.click(await ui.findByRole("button", { name: "Create" }));

    const alert = await ui.findByRole("alert");
    expect(within(alert).getByText("API error envelope")).toBeTruthy();
    expect(within(alert).getByText(/code: INVALID_BODY/)).toBeTruthy();
    expect(within(alert).getByText(/path: \/tasks/)).toBeTruthy();
